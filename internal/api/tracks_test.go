...
func (t *trackRepoMock) Search(query string, page, limit int) ([]*db.Track, int, error) {
	args := t.Called(query, page, limit)
...
func TestTracksApiService_TracksGet(t *testing.T) {
	t.Run("successful retrieval", func(t *testing.T) {
		...
		m := new(trackRepoMock)
		m.On("Search", query, int(page), int(limit)).Return(tracks, totalResults, nil)
		trackApi := TracksApiService{repo: m}
	...
	t.Run("transforms query keywords", func(t *testing.T) {
		...
		for _, testCase := range tests {
			t.Run(fmt.Sprintf("query %s", testCase.query), func(t *testing.T) {
				...
				m := new(trackRepoMock)
				m.On("Search", testCase.expectedQuery, 1, 10).Return(tracks, 1, nil)
				trackApi := TracksApiService{repo: m}
	...
	t.Run("no results found", func(t *testing.T) {
		...
		m := new(trackRepoMock)
		m.On("Search", mock.Anything, mock.Anything, mock.Anything).Return(tracks, 0, mongo.ErrNoDocuments)
		trackApi := TracksApiService{repo: m}
	...
	t.Run("database error", func(t *testing.T) {
		...
		m := new(trackRepoMock)
		m.On("Search", mock.Anything, mock.Anything, mock.Anything).Return(tracks, 0, databaseErr)
		trackApi := TracksApiService{repo: m}
...
func TestTracksApiService_TracksIdPatch(t *testing.T) {
	...
	t.Run("authenticated access", func(t *testing.T) {
		...
		td, _ := res.Body.(openapi.TrackDetail)
		assert.Equal(t, td.Lyrics, "new lyrics")
